...
export function SmartLearningDashboard({ studies, flashcards, quizzes }: {
  ...
  const { profile, knowledge, priorities, dailyPlan } = useLearningEngine(studies, flashcards, quizzes);
  const { graphs } = useKnowledgeGraphs();
  const analyticsInput = useMemo(() => ({ studies, profile, knowledge, flashcards, quizzes, graphs }), [flashcards, graphs, knowledge, profile, quizzes, studies]);
  const { snapshot: analytics, isLoading: analyticsLoading } = useAnalytics(analyticsInput);
  ...
  return (
    <section aria-labelledby="learning-dashboard-title" className="space-y-4">
      ...
      {analytics ? <AnalyticsDashboard snapshot={analytics} /> : analyticsLoading ? <div aria-label="Carregando Analytics" className="h-72 animate-pulse rounded-2xl border bg-muted/30" /> : null}
