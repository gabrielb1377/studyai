...
const preferencePrefixes = ["studyai:workspace", "studyai:ai-settings", "studyai-theme", "studyai:sidebar", "studyai:language"];
...
async function saveMeta(meta: SyncMetadata) { const next = { ...meta, state: { ...meta.state, pending: meta.queue.length }, updatedAt: new Date().toISOString() }; await StorageManager.put("metadata", next, { notify: false }); window.dispatchEvent(new CustomEvent("studyai:cloud-sync", { detail: next.state })); return next; }
  ...
  async syncNow() {
    ...
    const runCycle = async () => {
      ...
      try {
        ...
        meta = await saveMeta({ ...meta, state: { ...meta.state, status: "idle", cursor: pulled.cursor, lastSyncAt: new Date().toISOString(), uploadedBytes: meta.state.uploadedBytes + uploadedBytes } });
