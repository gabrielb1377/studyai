...
import { hasStructuredStudyContent } from "./services/StudyEngine";
...
export function StudyToolsTab({ study }: { study: StudyRecord }) {
  ...
  const hasStructuredContent = hasStructuredStudyContent(study);
  return (
    <section aria-labelledby="study-tools-title" className="space-y-5">
        ...
        {tools.map((tool) => {
          ...
          return (
            <Card key={tool.id} className="gap-0 p-5 shadow-none">
              ...
              <CardContent className="mt-5 p-0"><Button type="button" variant="outline" className="w-full" disabled={tool.id !== "notes" && !hasStructuredContent} title={tool.id !== "notes" && !hasStructuredContent ? "Disponível após a análise do conteúdo" : undefined} onClick={() => { if (tool.id === "flashcards") setIsFlashcardWorkspaceOpen(true); if (tool.id === "quiz") setIsQuizWorkspaceOpen(true); if (tool.id === "notes") setIsNotesWorkspaceOpen(true); }}>Abrir</Button></CardContent>
            </Card>
          );
        })}
      </div>
      {!hasStructuredContent && (
        <p className="rounded-lg border border-dashed p-3 text-sm text-muted-foreground">
          Flashcards e Quiz serão habilitados assim que o conteúdo estruturado deste estudo estiver disponível.
        </p>
      )}
