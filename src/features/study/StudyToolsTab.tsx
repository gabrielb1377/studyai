        ...
        {tools.map((tool) => {
          ...
          return (
            <Card key={tool.id} className="gap-0 p-5 shadow-none">
              ...
              <CardContent className="mt-5 p-0"><Button type="button" variant="outline" className="w-full" disabled={tool.id !== "flashcards"} onClick={() => tool.id === "flashcards" && setIsFlashcardWorkspaceOpen(true)}>Abrir</Button></CardContent>
            </Card>
          );
        })}
      </div>
