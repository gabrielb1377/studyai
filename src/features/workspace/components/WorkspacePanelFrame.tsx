...
export const WorkspacePanelFrame = memo(function WorkspacePanelFrame({ panel, children, onActivate, onMinimize, onMaximize, onClose, onScrollChange }: {
  ...
  return (
    <section role="tabpanel" aria-label={panel.title} className={`flex min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border bg-background shadow-sm ${panel.maximized ? "h-[calc(100dvh-11rem)]" : "h-[calc(100dvh-14rem)] min-h-[34rem] max-h-[58rem]"}`} onPointerDown={onActivate}>
      <header className="flex h-11 shrink-0 items-center gap-2 border-b bg-muted/35 px-3">
      ...
      {!panel.minimized && <div ref={bodyRef} className="min-h-0 flex-1 overflow-auto p-3 sm:p-4" onScroll={(event) => { window.clearTimeout(scrollTimer.current); const scrollTop = event.currentTarget.scrollTop; scrollTimer.current = window.setTimeout(() => onScrollChange(scrollTop), 180); }}>{children}</div>}
    </section>
  );
});
